...
async function getData() {
  return await fetch(
    process.env.PAYLOAD_PUBLIC_SERVER_URL +
      "/api/pages/64679656aa702068d83315cf",
  ).then((res) => res.json());
}

async function getTecnologias() {
  return await fetch(
    process.env.PAYLOAD_PUBLIC_SERVER_URL + "/api/tecnologias?limit=0",
  ).then((res) => res.json());
}
  ...
  return (
    <>
      ...
      <div className="py-20 space-y-6">
        {tecnologias.docs.map(
          (item: { TituloDeSatck: string; Progreso: number }, i: number) => {
            return (
              <div key={i} className="space-y-1">
                <Progresbar
                  stack={item.TituloDeSatck}
