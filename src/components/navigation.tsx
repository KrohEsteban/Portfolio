      ...
      <div className="py-2 bg-gris-intermedio">
        <nav className="md:flex md:justify-around relative">
          <div className="flex items-center justify-around">
            <Link className="text-4xl text-inherit" href="/">
              <span className="font-dancingscript">Esteban Kroh</span>
            </Link>
            <button
              className="flex justify-center h-10 w-10 md:hidden aria-expanded:toggle-btn"
              aria-label="Menu"
              type="button"
          ...
          >
            <ul className="w-3/4 md:w-auto bg-gris-intermedio bg-opacity-80 md:bg-inherit md:flex text-base md:text-gray-400 space-y-2 space-x-0 md:space-y-0 md:space-x-2 ">
              <li>
                <Link
                  href="/"
                  className="flex justify-end md:w-auto border-b md:border border-gris-oscuro shadow-md shadow-amarillo md:border-y-transparent md:hover:bg-gris-oscuro md:hover:border-y-gray-500 p-6 md:py-4 text-inherit"
                  onClick={() => {
                    menu === menuescondido
                      ? setMenu(menuvisible)
                      : setMenu(menuescondido);
              ...
              <li>
                <Link
                  href="/hobby"
                  className="flex justify-end md:w-auto border-b md:border border-gris-oscuro shadow-md shadow-amarillo md:border-y-transparent md:hover:bg-gris-oscuro md:hover:border-y-gray-500 p-6 md:p-4 text-inherit"
                  onClick={() => {
                    menu === menuescondido
                      ? setMenu(menuvisible)
                      : setMenu(menuescondido);
              ...
              <ul className="flex justify-center space-x-6 md:hidden py-10">
                {props.contactos.docs.map(
                  (item: { Nombre: string; Url: string; Svg: string }) => {
                    return (
                      <li className="w-10 sm:w-11 md:w-12" key={item.Nombre}>
                        <a
                          className="stroke-none text-amarillo p-4 m-auto"
                          href={item.Url}
                          target="_blank"
