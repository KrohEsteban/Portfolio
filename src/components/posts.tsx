...
export default function Post(props: { etiquetas: any; blogs: any }) {
  ...
  return (
    <>
      <div className="my-20">
        <p className="text-start text-lg sm:text-xl lg:text-2xl p-3">
        ...
        <div className="flex flex-wrap">
          <button
            ...
            className="p-5 m-3 rounded-lg shadow-sm border border-gris-claro bg-gris-intermedio hover:bg-gris-oscuro text-gris-claro"
          >
            Todos
          </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 py-5">
            ...
            if (select === "") {
              return (
                <Link
                  key={i}
                  href={`/blog/${item.Slug}`}
                  className="space-y-3 p-5 m-3 shadow-[-10px_-10px_10px_-10px] shadow-amarillo text-gris-claro"
                >
                  <p className="text-start text-lg sm:text-xl lg:text-2xl p-2">
                    {item.Title}
                  </p>
          ...
          {nopost === props.blogs.docs.length ? (
            <button className="space-y-3 p-5 m-3 shadow-[-10px_-10px_10px_-10px] shadow-amarillo text-gris-claro">
              <p className="text-start text-lg sm:text-xl lg:text-2xl p-2">
                No se encontraron post con esta etiqueta
              </p>
            </button>
          ) : (
            <></>
