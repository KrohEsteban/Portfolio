...
export default function Bloques(children: any): any {
  ...
  return children.map((node: any, i: number) => {
    if (node.blockType === "RichTextNormal") {
      return (
        <Fragment key={i}>
          <RichTextNormal>{node.RichTextNormal}</RichTextNormal>
        </Fragment>
      );
    } else if (node.blockType === "Modal") {
      return (
        <Fragment key={i}>
          <Modal>{node.Modal}</Modal>
        </Fragment>
      );
    } else if (node.blockType === "Carousel") {
      return (
        <Fragment key={i}>
          <Carousel>{node.Carousel}</Carousel>
        </Fragment>
      );
      ...
      return (
        <Fragment key={i}>
          <FotoBaner>{node.Image}</FotoBaner>
        </Fragment>
      );
    } else if (node.blockType === "RelacionProyectos") {
      return (
        <Fragment key={i}>
          <Proyectos>{node.ProyectoRelacionado}</Proyectos>
        </Fragment>
      );
    } else if (node.blockType === "DosColumnas") {
      return (
        <Fragment key={i}>
          <DosColumnas>{node}</DosColumnas>
        </Fragment>
      );
    } else {
      return null;
